...
//
// The mock ICD does not validate, so replay exits cleanly whether or not it dropped those structs. Compare what
// replay handed to the driver against a known good instead.
TEST(ExternalMemoryFD, RebindReplay)
{
    ...
    // Wait for the exporter to be ready
    sleep(1);
    verify_gfxr("external-memory-fd-import", nullptr, false, "rebind", { "-m", "rebind" });
}
