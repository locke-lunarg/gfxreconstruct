 ...
 *
 * This lets a test assert on what replay actually handed to the driver. capture_and_replay() only checks that
 * replay exited cleanly, which the mock ICD will do even when replay has dropped part of a pNext chain.
 *
 * @param test_name          - the name of the test app to launch and capture
